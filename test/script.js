function evaluateQuiz() {
    ...
    let resultPersonalTraits = resultMajors.map(major => answers.personalTraits[major]);
    let resultCareerAspirations = resultMajors.map(major => answers.careerAspirations[major]);
    let resultLeisurePreferences = resultMajors.map(major => answers.leisurePreferences[major]);
    let resultLearningStyles = resultMajors.map(major => answers.learningStyles[major]);

    // Display the results
    const resultDiv = document.getElementById('results');
    resultDiv.innerHTML = " Personal Traits: " + resultPersonalTraits.join(', ') + "<br>" +
                           " Career Aspirations: " + resultCareerAspirations.join(', ') + "<br>" +
                           " Leisure Preferences: " + resultLeisurePreferences.join(', ') + "<br>" +
                           " Learning Styles: " + resultLearningStyles.join(', ') + "<br>" +
                           " Majors of Interest: " + resultMajors.join(', ');
}
